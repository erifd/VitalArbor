...
def convert_bitmap_to_png(bitmap_path):
    """
    Convert a bitmap file to PNG format in a temporary directory.
    Returns: path to the temporary PNG file or None if conversion fails
    """
    if not bitmap_path or not os.path.exists(bitmap_path):
        return None
    
    # Check if file is already PNG
    if bitmap_path.lower().endswith('.png'):
        print(f"File is already PNG: {bitmap_path}")
        return bitmap_path
    
    # Check if file is a bitmap
    if not bitmap_path.lower().endswith(('.bmp', '.bitmap')):
        print(f"File is not a bitmap, using as-is: {bitmap_path}")
        return bitmap_path
    
    try:
        # Create temporary file with PNG extension
        temp_fd, png_path = tempfile.mkstemp(suffix='_converted.png', prefix='tree_')
        os.close(temp_fd)  # Close the file descriptor
        
        # Convert using PIL
        print(f"Converting bitmap to PNG: {bitmap_path} -> {png_path}")
        img = Image.open(bitmap_path)
        img.save(png_path, 'PNG')
        print(f"Successfully converted to temporary PNG: {png_path}")
        
        return png_path
    except Exception as e:
        print(f"Error converting bitmap to PNG: {e}")
        return None
...
def main():
    ...
    photo_png = convert_bitmap_to_png(photo)
    tilt_photo_png = convert_bitmap_to_png(tilt_photo)
    
    # Track which files need cleanup (only if they were converted)
    if photo_png and photo_png != photo:
        temp_files_to_cleanup.append(photo_png)
    if tilt_photo_png and tilt_photo_png != tilt_photo:
        temp_files_to_cleanup.append(tilt_photo_png)
    
    # Validate conversions
    if photo and not photo_png:
        print(f"ERROR: Failed to convert classification photo: {photo}")
        cleanup_temp_files(temp_files_to_cleanup)
        return
    
    if tilt_photo and not tilt_photo_png:
        print(f"ERROR: Failed to convert tilt photo: {tilt_photo}")
        cleanup_temp_files(temp_files_to_cleanup)
        return
    
    # Use converted PNG paths
    photo = photo_png
    tilt_photo = tilt_photo_png
    ...
    while tilt is None:
        ...
        # Convert backup image if needed
        print("\n=== Converting Backup Image ===")
        backup_png = convert_bitmap_to_png(backup_path)
        
        # Track backup temp file if it was converted
        if backup_png and backup_png != backup_path:
            temp_files_to_cleanup.append(backup_png)
        
        if not backup_png:
            print("ERROR: Failed to convert backup image")
            continue
        
        # Segment the backup image
        print("\n=== Segmenting Backup Image ===")
        segmented_backup = segment_and_get_path(backup_png, "backup photo")
